...
@section('content')
<link href="{{ asset('css/playlists/edit.css') }}" rel="stylesheet">
...
<div id="notification-container">
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">{{ __('Edit Playlist') }}</div>
            ...
            <div class="card-body">
                <form action="{{ route('playlists.update', $playlist->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control" rows="4" required>{{ $playlist->description }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <ul id="selected_videos" class="list-group mb-3">
                            @foreach($playlist->videos as $video)
                                <li class="list-group-item" data-video-id="{{ $video->id }}">
                                    {{ $video->title }}
                                    <button class="btn btn-sm btn-danger float-right" onclick="removeVideo({{ $video->id }}, this)">Remove</button>
                                </li>
                            @endforeach
                        </ul> <!-- List of selected videos -->
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <div class="input-file-wrapper">
                                ...
                                <span id="file-name" class="file-name-text">{{ $playlist->image ? $playlist->image : 'No file chosen' }}</span> <!-- Display file name -->
                            </div>
                            @if($playlist->image)
                            <img src="{{ asset('storage/'.$playlist->image) }}" alt="Current post Image" class="img-thumbnail mt-2" style="max-width: 150px;">
                        @endif
                        </div>
                    </div>
                    ...
                    <script>
                        function showFileName() {
                            ...
                            var fileName = input.files[0] ? input.files[0].name : '{{ $playlist->image ? $playlist->image : 'No file chosen' }}'; // Show the file name or default
                    ...
                    <input type="hidden" name="video_ids" id="video_ids" value="{{ json_encode($playlist->videos->pluck('id')) }}">

                    <button type="submit" class="btn-submit">{{ __('Update Playlist') }}</button>
                </form>
            </div>
        </div>
    </div>
